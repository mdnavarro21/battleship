...
const DomController = (() => {
    ...
    const onTileClick = (e) => {
        ...
        const p1_attack = [parseInt(dom_tile.getAttribute('data-column')), parseInt(dom_tile.getAttribute('data-row'))];
        const condition = Game.playRound(p1_attack);
        if (condition) {
            const game_tile = Game.player2.gameBoard.getBoard()[parseInt(dom_tile.getAttribute('data-row'))][parseInt(dom_tile.getAttribute('data-column'))]
    ...
    const renderShipyard = () => {
        ...
        const shipIDs = ['Patrol-Boat', 'Submarine','Destroyer','Battleship', 'Carrier'];
        const shipImages = ['https://static.thenounproject.com/png/9270-200.png',
    ...
    const Drag_and_Drop = (() => {
        ...
        const drop = (e) => {
            ...
            const shipLength = data == 'Carrier' ? 5
            : data == 'Battleship' ? 4
            : data == 'Destroyer' ? 3
            : data == 'Submarine' ? 2
            : 1;
            Game.player1.gameBoard.placeShip(ShipFactory(shipLength), [parseInt(e.currentTarget.getAttribute('data-column')),parseInt(e.currentTarget.getAttribute('data-row'))], 'horizontal');
            updateBoard(Game.player1);
            document.querySelector(`#shipyard > div >#${data}`).parentElement.remove();
            if (document.getElementById('shipyard').hasChildNodes() == false) {
                document.querySelector('#shipyard').remove();
                document.querySelector('#p2-gameboard-container').append(renderGameBoard(Game.player2));
